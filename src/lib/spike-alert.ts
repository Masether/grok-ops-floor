...
/** Spike sized to clear fees on the take — not a micro tick. */
export function hugeSpike(input: {
  ...
  const tapeRip = input.oneMinPct >= 0.9 && input.volRatio >= 1.45;
  const tapeBlast = input.threePct >= 1.8 && input.oneMinPct >= 0.35 && input.volRatio >= 1.25;
  if (tapeRip || tapeBlast) return { ok: true, source: "tape" };
  const wireFresh = input.wireKind === "trend" && (input.wireAgeMs ?? 1e12) < 30 * 60_000;
  if (wireFresh && input.oneMinPct >= 0.55 && input.lane === "up") {
